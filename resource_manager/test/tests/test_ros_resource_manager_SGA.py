...
class ResourceManagerROSTests(unittest.TestCase):
    ...
    def test_request_bad_behavior_name(self):
        """
        Verify that a resource request fails when a behavior that is not loaded
        requests a resource.
        """
        response1 = self.request_resources(
                "behavior_-1", 10, self.test_request.roles)
        self.assertFalse(response1.success)
    ...
    def test_failed_service_call_reassignment(self):
        """
        Verify that when a service call fails the resource manager still
        correctly assigns resources.
        """
        bad_request = copy.deepcopy(self.test_request)
        bad_request.roles[0].name = 'role_230230'
        # Send request that should fail
        response1 = self.request_resources(
                "behavior_2", 10, bad_request.roles)
        # Send request that shouldn't fail
        response2 = self.request_resources(
                "behavior_1", 10, self.test_request.roles)
        # Check to see if they correctly did and did not fail
        self.assertFalse(response1.success)
        self.assertTrue(response2.success)
        self.assertEqual(len(response2.roles), 2)
